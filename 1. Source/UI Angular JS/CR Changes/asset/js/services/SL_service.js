App.service('SL_SERVICE', ['$http', 'util_SERVICE', function ($http, US) {
   ...
   .success(function (response) {
    ...
    //get surveyor Agent
    this.getSL = function (sno,orderno,cardcode,cardname,survydatefrom,survydateTo,containerno) {
        ...
        var data = {  
            "ODLN": [{  
                ...
                "U_Ccode": cardcode,
                "U_Cname": cardname,
                "U_SrvyDatFrm": survydatefrom,
                "U_SrvyDatTo": survydateTo,
                "U_ContainerNo":containerno
            }] 
        }
        var parms = encodeURIComponent(JSON.stringify(data));
        var promise = $http.post(this.url + "GetSurveyList", "value=" + parms, this.config)
   .success(function (response) {
       if (response.returnStatus == 1) {
           return response;
       } else {
	...
	this.getsurveyno = function(sno,uid) {
		var data = {
            "ODLN": [{
                "U_SurvyNo": sno,
                "uid": uid
		...
		var parms = encodeURIComponent(JSON.stringify(data));
		var promise = $http.post(this.url + "GetSurveyNo","value=" + parms,this.config)
		.success(function (response){
						   if (response.returnStatus == 1){
							   return response;
						   }
    ...
    this.getSalesOrderNo = function(sno,uid){
        var data = {
            "ODLN": [{
                "U_OrderNo": sno,
                "uid": uid
        ...
        var parms = encodeURIComponent(JSON.stringify(data));
        var promise = $http.post(this.url + "GetSalesOrderNo","value=" +parms,this.config)
        .success(function(response){
                if (response.returnstatus==1){
                return respon;
                }
