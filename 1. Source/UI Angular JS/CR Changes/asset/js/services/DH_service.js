App.service('DH_SERVICE', ['$http', 'util_SERVICE', function ($http, US) {
   ...
   .success(function (response) {
    ...
    //get surveyor Agent
    this.getDH = function (sno) {

        var data = {  "ODLN": [{  "U_EqNo": sno }] };
        var parms = encodeURIComponent(JSON.stringify(data));
        var promise = $http.post(this.url + "DamageHistory_Full", "value=" + parms, this.config)
   .success(function (response) {
       if (response.returnStatus == 1) {
           return response;
       } else {
