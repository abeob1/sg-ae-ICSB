App.controller('ONHS_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','ONHS_SERVICE','hotkeys','$location',

function ($scope, $rootScope, $http, $window, $cookies,US,ONHS,hotkeys,$location) {
   ...
   $rootScope.FindoneRecord = function(code)
    {   
         ...
         ONHS.FindRecord(code).then(function (response) {console.log(response);$scope.data=response.data;
         $scope.updateclick();
   ...
   //adding shortcuts
    ...
    ONHS.getetype().then(function (response) { $scope.etype = response.data.SURCRTA; });
    ONHS.getSurveyCriteria().then(function (response) { $scope.SurveyCriteria = response.data.SURCRTA;  });
    ONHS.getSurveyorIDMaster().then(function (response) { $scope.SurveyorIDMaster = response.data.SUID; });//get full surveyer
    ...
    if(type=='code')
    {
        ...
        n=$scope.data.ODLN[0].CardName;
         ...
         ONHS.getcustomer(c,n).then(function (response) { $rootScope.customer = response.data; $("#myModal").modal(); 
                });
    }
     ...
     $scope.getsurveyorID = function()
    {
         ...
         ONHS.getsurveyorID($scope.data.ODLN[0].U_UName).then(function (response) { $rootScope.surveyor = response.data; $("#myModal2").modal(); 
                });
    }
    ...
    $scope.getsurveyAgent = function()
    {
         ...
         ONHS.getsurveyAgent($scope.UserData[0].Code).then(function (response) { $rootScope.surveyorAgent = response.data; $("#Modal3").modal(); 
                });
    }
    ...
    $scope.PreviousRecord = function()
    {
         ...
         ONHS.PreviousRecord($scope.data.ODLN[0].U_OrderNo,$scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data; $scope.updateclick();});
    }
    ...
    $scope.NextRecord = function()
    {
         ...
         ONHS.NextRecord($scope.data.ODLN[0].U_OrderNo,$scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    $scope.LastRecord = function()
    {
         ...
         ONHS.LastRecord($scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    $scope.FirstRecord = function()
    {
         ...
         ONHS.FirstRecord($scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    {   
         ...
         ONHS.FindMultiRecord($scope.data.SQTO[0]).then(function (response) {$rootScope.findresult = response.data; console.log($rootScope.findresult); $("#findmodal").modal();
   ...
   //validation function
    ...
    $scope.save = function()
    {
    $scope.data.ODLN[0].U_FormName = "ONHS";
    if($scope.val())
    {
        ...
        $http.post(US.url+"OnHire_SurveyTyepAdd", "value=" + parms, config)
   .then(
       function (response) {
           // success callback
    ...
    $scope.SQM_Approval = function()
    {
        ...
        //ONHS.SQMApproval($scope.data.SQTO.U_Qno).then(function (response) { console.log(response); });
        ONHS.aproval($scope.data.SQTO[0].U_Qno).then(function (response) {
     ...
     $rootScope.Reject = function()
    {
        ...
        ONHS.approver($scope.data.SQTO[0].U_Qno,'Rejected',remarks,uname).then(function (response) {
        if (response.data.APPROVAL[0].Status == "True") {             
              // $scope.addclick();
              alert(response.data.APPROVAL[0].Msg);
   ...
   .then(
    ...
    if($scope.val())
    {
        ...
        var parms = encodeURIComponent(JSON.stringify($scope.data));
        $http.post(US.url+"OnHire_SurveyTyepUpdate", "value=" + parms, config)
   .then(
       function (response) {
           // success callback
           console.log(response.data);
