App.controller('SurveyList_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','SL_SERVICE','hotkeys','$location',
...
function ($scope, $rootScope, $http, $window, $cookies,US,SL,hotkeys,$location) {
   ...
   // SL.getSurveyorIDMaster().then(function (response) { $scope.SurveyorIDMaster = response.data.SUID; });//get full surveyer
	...
	$scope.getsurveyno = function(SurveyNo)
	{
		//SL.getsurveyno(SurveyNo, $scope.UserData[0].Code).then(function(response){$scope.U_SurvyNo = response.data.U_SurveyNo; console.log($scope.U_SurvyNo); })
        SL.getsurveyno(SurveyNo, $scope.UserData[0].Code).then(function(response){$rootScope.U_SurvyNo = response.data; $("#myModalSuveyNo").modal(); })
	}

    $rootScope.FillSurveyNo = function(SurveyNo)
    {
        $scope.U_SurvyNo = SurveyNo;
    }

    $scope.getSalesOrderNo = function(SODocNum)
    {
        SL.getSalesOrderNo(SODocNum, $scope.UserData[0].Code).then(function(response){$rootScope.U_OrderNo = response.data; $("#myModalSONum").modal(); })
    }

    $rootScope.fillSalesOrderNo = function(OrderNo)
    {
        $scope.U_OrderNo = OrderNo;
    }
    ...
    $scope.findSL = function()
    {
        ...
        SL.getSL($scope.U_SurvyNo,$scope.U_OrderNo,$scope.U_Ccode,$scope.U_Cname,$scope.U_SrvyDatFrm,$scope.U_SrvyDatTo,$scope.U_ContainerNo).then(function (response) { $scope.SL = response.data.ODLN;  });//get full SL
    ...
    {
    //alert($scope.data.ODLN[0].CardCode);
    var c,n;
    if(type=='code')
    {
        c = $scope.U_Ccode;
        ...
        n=$scope.U_Cname;
         ...
         SL.getcustomer(c,n,$scope.UserData[0].Code).then(function (response) { $rootScope.customer = response.data; $("#myModal").modal(); 
                });
    }
