App.controller('OFHS_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','OFHS_SERVICE','hotkeys','$location',

function ($scope, $rootScope, $http, $window, $cookies,US,OFHS,hotkeys,$location) {
   ...
   $rootScope.FindoneRecord = function(code)
    {   
         ...
         OFHS.FindRecord(code).then(function (response) {console.log(response);$scope.data=response.data;
         $scope.updateclick();
   ...
   //adding shortcuts
    ...
    OFHS.getetype().then(function (response) { $scope.etype = response.data.SURCRTA; });
    OFHS.getSurveyCriteria().then(function (response) { $scope.SurveyCriteria = response.data.SURCRTA;  });
    OFHS.getSurveyorIDMaster().then(function (response) { $scope.SurveyorIDMaster = response.data.SUID; });//get full surveyer
    ...
    if(type=='code')
    {
        ...
        n=$scope.data.ODLN[0].CardName;
         ...
         OFHS.getcustomer(c,n).then(function (response) { $rootScope.customer = response.data; $("#myModal").modal(); 
                });
    }
     ...
     $scope.getsurveyorID = function()
    {
         ...
         OFHS.getsurveyorID($scope.data.ODLN[0].U_UName).then(function (response) { $rootScope.surveyor = response.data; $("#myModal2").modal(); 
                });
    }
    ...
    $scope.getsurveyAgent = function()
    {
         ...
         OFHS.getsurveyAgent($scope.UserData[0].Code).then(function (response) { $rootScope.surveyorAgent = response.data; $("#Modal3").modal(); 
                });
    }
    ...
    $scope.PreviousRecord = function()
    {
         ...
         OFHS.PreviousRecord($scope.data.ODLN[0].U_OrderNo,$scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data; $scope.updateclick();});
    }
    ...
    $scope.NextRecord = function()
    {
         ...
         OFHS.NextRecord($scope.data.ODLN[0].U_OrderNo,$scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    $scope.LastRecord = function()
    {
         ...
         OFHS.LastRecord($scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    $scope.FirstRecord = function()
    {
         ...
         OFHS.FirstRecord($scope.UserData[0].Code).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    {   
         ...
         OFHS.FindMultiRecord($scope.data.SQTO[0]).then(function (response) {$rootScope.findresult = response.data; console.log($rootScope.findresult); $("#findmodal").modal();
...
$scope.sum = function()
{
$scope.data.ODLN[0].U_Esti_Diff = $scope.data.ODLN[0].U_Esti_Org - $scope.data.ODLN[0].U_Esti_Rev;
$scope.data.ODLN[0].U_Curr_diff = $scope.data.ODLN[0].U_Curr_Org - $scope.data.ODLN[0].U_Curr_Rev;
$scope.data.ODLN[0].U_Impac_diff = $scope.data.ODLN[0].U_Impac_Org - $scope.data.ODLN[0].U_Impac_Rev;
$scope.data.ODLN[0].U_Wear_Diff = $scope.data.ODLN[0].U_Wear_org - $scope.data.ODLN[0].U_Wear_Rev;
$scope.data.ODLN[0].U_Cust_Diff = $scope.data.ODLN[0].U_Cust_Org - $scope.data.ODLN[0].U_Cust_Rev;
$scope.data.ODLN[0].U_Import_Diff = $scope.data.ODLN[0].U_Impor_Org - $scope.data.ODLN[0].U_Impor_Rev;

$scope.data.ODLN[0].U_Sum_Org = parseFloat($scope.data.ODLN[0].U_Esti_Org) + parseFloat($scope.data.ODLN[0].U_Curr_Org) + parseFloat($scope.data.ODLN[0].U_Impac_Org) + parseFloat($scope.data.ODLN[0].U_Wear_org) + parseFloat($scope.data.ODLN[0].U_Cust_Org) + parseFloat($scope.data.ODLN[0].U_Impor_Org);
$scope.data.ODLN[0].U_Sum_Rev = parseFloat($scope.data.ODLN[0].U_Esti_Rev) + parseFloat($scope.data.ODLN[0].U_Curr_Rev) + parseFloat($scope.data.ODLN[0].U_Impac_Rev) + parseFloat($scope.data.ODLN[0].U_Wear_Rev) + parseFloat($scope.data.ODLN[0].U_Cust_Rev) + parseFloat($scope.data.ODLN[0].U_Impor_Rev);
$scope.data.ODLN[0].U_Sum_Diff = $scope.data.ODLN[0].U_Sum_Org - $scope.data.ODLN[0].U_Sum_Rev;
}
$scope.sum();
   ...
   .then(
    ...
    if($scope.val())
    {
        ...
        $http.post(US.url+"OffHire_SurveyTyepAdd", "value=" + parms, config)
   .then(
       function (response) {
           // success callback
           console.log(response.data);
    ...
    if($scope.val())
    {
        ...
        var parms = encodeURIComponent(JSON.stringify($scope.data));
        $http.post(US.url+"OffHire_SurveyTyepUpdate", "value=" + parms, config)
   .then(
       function (response) {
           // success callback
           console.log(response.data);
    ...
    $scope.SQM_Approval = function()
    {
        ...
        //OFHS.SQMApproval($scope.data.SQTO.U_Qno).then(function (response) { console.log(response); });
        OFHS.aproval($scope.data.SQTO[0].U_Qno).then(function (response) {
     ...
     $rootScope.Reject = function()
    {
        ...
        OFHS.approver($scope.data.SQTO[0].U_Qno,'Rejected',remarks,uname).then(function (response) {
        if (response.data.APPROVAL[0].Status == "True") {             
              // $scope.addclick();
              alert(response.data.APPROVAL[0].Msg);
