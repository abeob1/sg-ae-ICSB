App.controller('CustomerContract_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','CS_SERVICE','hotkeys',

function ($scope, $rootScope, $http, $window, $cookies,US,CQS,hotkeys) {
    ...
    $scope.duplicateclick = function()
    {
         ...
         $scope.data.CCON[0].U_Qno = "";
         $scope.data.CCON[0].U_QutoNo = "";
         
        $scope.data.CCON[0].U_Status = "";
        $scope.data.CCON[0].U_AddrN = "";
        $scope.data.CCON[0].U_Addr1 = "";
        $scope.data.CCON[0].U_Addr2 = "";
        $scope.data.CCON[0].U_Addr3 = "";
        $scope.data.CCON[0].U_Addr4 = "";
        $scope.data.CCON[0].U_Addr5 = "";
        $scope.data.CCON[0].U_Addr6 = "";
        $scope.data.CCON[0].U_TelNo = "";
        $scope.data.CCON[0].U_FaxNo = "";
        $scope.data.CCON[0].U_Mno = "";
        $scope.data.CCON[0].U_Email = "";
        $scope.data.CCON[0].U_Cname = "";
        $scope.data.CCON[0].U_Ccode = "";
    ...
    if(type=='code')
    {
        c = $scope.data.CCON[0].U_Ccode;
        n="";
    }
    else{
        c =""; 
        n=$scope.data.CCON[0].U_Cname;
    }
    
         CQS.getcustomer(c,n).then(function (response) { $rootScope.customer = response.data; console.log($rootScope.customer); $("#myModal").modal(); 
                $scope.data.CCON[0].U_Addr1 ="";
                $scope.data.CCON[0].U_Addr2 ="";
                $scope.data.CCON[0].U_Addr3 ="";
                $scope.data.CCON[0].U_Addr4 ="";
    ...
    $scope.PreviousRecord = function()
    {
         ...
         CQS.PreviousRecord($scope.data.CCON[0].U_Qno).then(function (response) {console.log(response);$scope.data=response.data; $scope.updateclick();});
    }
    ...
    $scope.NextRecord = function()
    {
         ...
         CQS.NextRecord($scope.data.CCON[0].U_Qno).then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    $scope.LastRecord = function()
    {
         ...
         CQS.LastRecord().then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    $scope.FirstRecord = function()
    {
         ...
         CQS.FirstRecord().then(function (response) {console.log(response);$scope.data=response.data;$scope.updateclick();});
    }
    ...
    {   
         ...
         CQS.FindMultiRecord($scope.data.CCON[0]).then(function (response) {$rootScope.findresult = response.data; console.log($rootScope.findresult); $("#findmodal").modal();
   ...
   $rootScope.FindoneRecord = function(code)
    {   
         ...
         CQS.FindRecord(code).then(function (response) {console.log(response);$scope.data=response.data;
         $scope.updateclick();
    ...
    $rootScope.findone = function (i) {
        ...
        $scope.CCselected = true;
        $scope.data.CCON[0].U_Ccode = $rootScope.customer.OCRD[i].U_Ccode;
        $scope.data.CCON[0].U_Cname = $rootScope.customer.OCRD[i].U_Cname;
        $scope.data.CCON[0].U_TelNo = $rootScope.customer.OCRD[i].U_TelNo;
        $scope.data.CCON[0].U_FaxNo = $rootScope.customer.OCRD[i].U_FaxNo;
        $scope.data.CCON[0].U_Mno = $rootScope.customer.OCRD[i].U_MNo;
        $scope.data.CCON[0].U_Email = $rootScope.customer.OCRD[i].U_Email;
   ...
   $scope.setaddress = function(name)
   {
         for (var i=0; i<$scope.addressarray.length; i++) {
          ...
          {
                $scope.data.CCON[0].U_Addr1 =$scope.addressarray[i].U_Addr1;
                $scope.data.CCON[0].U_Addr2 =$scope.addressarray[i].U_Addr2;
                $scope.data.CCON[0].U_Addr3 =$scope.addressarray[i].U_Addr3;
                $scope.data.CCON[0].U_Addr4 =$scope.addressarray[i].U_Addr4;
                $scope.data.CCON[0].U_Addr5 =$scope.addressarray[i].U_Addr5;
                $scope.data.CCON[0].U_Addr6 =$scope.addressarray[i].U_Addr6;
            }
        }
   }
    ...
    if($scope.genval())
        ...
        $scope.data.CCONGEN.push(angular.copy($scope.gen));
        console.log($scope.data.CCONGEN);
        $scope.g_reset();
        }
    }
    ...
    $scope.g_edit = function(i)
    {
       // alert(i);
        ...
        $scope.data.CCONGEN[i].E_Conti = $scope.getcontijson($scope.data.CCONGEN[i].U_Conti);
        $scope.getcountry($scope.data.CCONGEN[i].E_Conti.U_Conti);

        $scope.data.CCONGEN[i].E_Country = $scope.getcountryjson($scope.data.CCONGEN[i].U_Country);
        $scope.getcity($scope.data.CCONGEN[i].E_Country.U_Country);
        
        $scope.data.CCONGEN[i].E_City = $scope.getcityjson($scope.data.CCONGEN[i].U_City);
        
    }
    $scope.g_ok = function(i)
    {
       console.log( $scope.data.CCONGEN[i].E_Conti);
        $scope.data.CCONGEN[i].edit = false;
        $scope.data.CCONGEN[i].U_Conti= angular.copy( $scope.data.CCONGEN[i].E_Conti.U_Conti);
        $scope.data.CCONGEN[i].U_Country= angular.copy( $scope.data.CCONGEN[i].E_Country.U_Country);
        $scope.data.CCONGEN[i].U_City= angular.copy( $scope.data.CCONGEN[i].E_City.U_City);

        $scope.data.CCONGEN[i].C_Conti= angular.copy( $scope.data.CCONGEN[i].E_Conti);
        $scope.data.CCONGEN[i].C_Country= angular.copy( $scope.data.CCONGEN[i].E_Country);
        $scope.data.CCONGEN[i].C_City= angular.copy( $scope.data.CCONGEN[i].E_City);
    ...
    {  
            ...
            $scope.g_Continent= $scope.getcontijson($scope.data.CCONGEN[i].U_Conti);
            $scope.getcountry($scope.g_Continent.U_Conti);
            $scope.g_Country=  $scope.getcountryjson($scope.data.CCONGEN[i].U_Country);
            $scope.getcity($scope.g_Country.U_Country);
            $scope.g_city= $scope.getcityjson($scope.data.CCONGEN[i].U_City);
            ...
            $scope.g_Rrate= $scope.data.CCONGEN[i].U_RRate;
    ...
    if($scope.addval())
    {
        ...
        $scope.data.CCONADD.push(angular.copy($scope.add));
        $scope.a_reset();
     }
    }
    ...
    $scope.a_edit = function(i)
    {
       // alert(i); 
        ...
        $scope.data.CCONADD[i].Ea_Conti = $scope.getcontijson($scope.data.CCONADD[i].U_Continent);
        $scope.getcountry($scope.data.CCONADD[i].Ea_Conti.U_Conti);

        $scope.data.CCONADD[i].Ea_Country = $scope.getcountryjson($scope.data.CCONADD[i].U_Country);
        $scope.getcity($scope.data.CCONADD[i].Ea_Country.U_Country);

        $scope.data.CCONADD[i].Ea_City = $scope.getcityjson($scope.data.CCONADD[i].U_City);
        $scope.getlocation($scope.data.CCONADD[i].Ea_City.U_City);

        $scope.data.CCONADD[i].Ea_EQGroup = $scope.getlocationjson($scope.data.CCONADD[i].U_EQGroup);
    ...
    $scope.a_ok = function(i)
    {
        ...
        $scope.data.CCONADD[i].U_Continent= angular.copy( $scope.data.CCONADD[i].Ea_Conti.U_Conti);
        $scope.data.CCONADD[i].U_Country= angular.copy( $scope.data.CCONADD[i].Ea_Country.U_Country);
        $scope.data.CCONADD[i].U_City= angular.copy( $scope.data.CCONADD[i].Ea_City.U_City);
        $scope.data.CCONADD[i].U_EQGroup= angular.copy( $scope.data.CCONADD[i].Ea_EQGroup.U_Loc);

        $scope.data.CCONADD[i].C_Conti= angular.copy( $scope.data.CCONADD[i].Ea_Conti);
        $scope.data.CCONADD[i].C_Country= angular.copy( $scope.data.CCONADD[i].Ea_Country);
        $scope.data.CCONADD[i].C_City= angular.copy( $scope.data.CCONADD[i].Ea_City);
    ...
    { 
            ...
            $scope.a_Continent= $scope.getcontijson($scope.data.CCONADD[i].U_Continent);
            $scope.getcountry($scope.data.CCONADD[i].U_Continent);

            $scope.a_Country=  $scope.getcountryjson($scope.data.CCONADD[i].U_Country);
            $scope.getcity($scope.data.CCONADD[i].U_Country);

            $scope.a_city= $scope.getcityjson($scope.data.CCONADD[i].U_City);
            $scope.getlocation($scope.data.CCONADD[i].U_City);

            $scope.a_curency= $scope.data.CCONADD[i].U_Currency;
            $scope.a_egroup= $scope.getlocationjson($scope.data.CCONADD[i].U_EQGroup);
    ...
    {
        for (var i=0; i<data.CCONGEN.length; i++) {
          delete data.CCONGEN[i].C_Conti;
          delete data.CCONGEN[i].C_Country;
          delete data.CCONGEN[i].C_City;

          delete data.CCONGEN[i].E_Conti;
          delete data.CCONGEN[i].E_Country;
          delete data.CCONGEN[i].E_City;
         if(data.CCONGEN[i].U_RRate===undefined || data.CCONGEN[i].U_RRate==null)
            data.CCONGEN[i].U_RRate=0;
         if(data.CCONGEN[i].U_Rate===undefined || data.CCONGEN[i].U_Rate==null)
            data.CCONGEN[i].U_Rate=0;
          data.CCONGEN[i].U_Rate = data.CCONGEN[i].U_Rate.toFixed(2);
          data.CCONGEN[i].U_RRate = data.CCONGEN[i].U_RRate.toFixed(2);
          
        }

        for (var i=0; i<data.CCONADD.length; i++) {
          delete data.CCONADD[i].C_Conti;
          delete data.CCONADD[i].C_Country;
          delete data.CCONADD[i].C_City;

          delete data.CCONADD[i].Ea_Conti;
          delete data.CCONADD[i].Ea_Country;
          delete data.CCONADD[i].Ea_City;
          delete data.CCONADD[i].Ea_EQGroup
           if(data.CCONADD[i].U_RRate===undefined || data.CCONADD[i].U_RRate==null)
            data.CCONADD[i].U_RRate=0;
         if(data.CCONADD[i].U_Rate===undefined || data.CCONADD[i].U_Rate==null)
            data.CCONADD[i].U_Rate=0;
          data.CCONADD[i].U_Rate = data.CCONADD[i].U_Rate.toFixed(2);
          data.CCONADD[i].U_RRate = data.CCONADD[i].U_RRate.toFixed(2);
    ...
    $scope.val  = function()
    {
     ...
     if($scope.data.CCONGEN.length== 0 || typeof $scope.data.CCONGEN==="undefined")
        {
            retval = false;
            errormsg = errormsg+". Kindly Add Record in General<br/>";
        }
        
         if($scope.data.CCON[0].U_Uname=="" || typeof $scope.data.CCON[0].U_Uname==="undefined")
        {
            retval = false;
            errormsg = errormsg+". Kindly Enter User Name<br/>";
        }
        if($scope.data.CCON[0].U_CPeriod1=="" || typeof $scope.data.CCON[0].U_CPeriod1==="undefined" || $scope.data.CCON[0].U_CPeriod2=="" || typeof $scope.data.CCON[0].U_CPeriod2==="undefined" )
        {
            retval = false;
            errormsg = errormsg+". Kindly Select Contract Period Date<br/>";
        }
        if($scope.data.CCON[0].U_Cdate=="" || $scope.data.CCON[0].U_Cdate=="undefined")
        {
            retval = false;
             errormsg = errormsg+". Kindly Select Create Date<br/>";//alert("Kindly");
           // bootbox.alert("Hello world!", null);
        }
        if($scope.data.CCON[0].U_Ccode=="" || $scope.data.CCON[0].U_Ccode=="undefined")
        {
            retval = false;
    ...
    if($scope.val())
    {
        ...
        $http.post(US.url+"CustomerContractAdd", "value=" + parms, config)
   .then(
       function (response) {
           // success callback
    ...
    if($scope.val())
    {
        ...
        var parms = encodeURIComponent(JSON.stringify($scope.formsavedata($scope.data)));
        $http.post(US.url+"CustomerContract_Update", "value=" + parms, config)
   .then(
       function (response) {
           // success callback
           console.log(response.data);
    ...
    //Keep It Here
    if(getParameterByName('CTC')=='true')
    {
       $scope.CTCdata = angular.fromJson($cookies.get('CTC_data'));
       $scope.data.CCON[0] = angular.copy($scope.CTCdata.SQTO[0]);
